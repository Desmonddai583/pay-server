        ...
        return async (ctx, next) => {
            try {
                ...
                await users.getUser2(data).then(result => {
                  ...
                  // 更新用户状态
                  console.log('执行用户状态更新')
                if (userData.membership_expired < new Date().getTime()){
                    // 会会员到期了
                    let data  = {
                    ...
                    await users.updateMem(data)
                }
                console.log('结束用户状态更新')
                if (data.payway == '2') {
                ...
                if (order.test == 2) {
                    ...
                    if(order.appid === "15c821df1e52") {
                        await users.updateBalance({action: 'deposit', userid: order.userid, amount: data.amount})
                        await deposits.updateDepositInfo({status: 1, amount: data.amount, orderid: data.orderid})
                    }
