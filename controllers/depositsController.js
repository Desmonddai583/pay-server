...
const domain = require('../conf/default').domain;
        ...
        return async (ctx, next) => {
            try {
                ...
                if (data.payway == '2') {
                    ...
                    let qrcode = `alipays://platformapi/startapp?appId=20000691&url=${domain}static%2Fpay%2Falipay.html%3Fu%3D${pay_userId}%26a%3D${amo}%26m%3D${orderid}`;
                  	data.qrcode = qrcode;
                    data.orderid = orderid;
                    await deposits.addDeposit(data);
